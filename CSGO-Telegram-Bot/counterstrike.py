...
import re
import unicodedata
from counterstrike_modules.player import cs_player
from counterstrike_modules.team import cs_team
import itertools
...
class counterstrike():
	...
	def get_upcoming_and_ongoing_games(self):
		...
		for match in matches:
			...
			try:
				game['team1'] = cells[0].find('span',class_='team-template-team-short').find('a').get('title')
				game['team2'] = cells[2].find('span',class_='team-template-team-short').find('a').get('title')	
				game['start_time'] = cells[3].find('span',class_="timer-object").get("data-timestamp")
	...
	def get_tournaments(self,tournamentType=None):
		...
		for table in tables:
			...
			for row in rows:
				...
				tournament["link"] = tournament_cell.find('b').find("a").get("href")
				...
				if "first_place" in tournament and tournament["first_place"] == "TBD":
